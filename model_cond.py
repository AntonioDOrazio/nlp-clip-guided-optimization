...
class ClipImageEmbeddingLoss(nn.Module):
    def __init__(self):
        super(ClipImageEmbeddingLoss, self).__init__()
    ...
    def forward(self, x, target):
        with torch.no_grad():
            target_enc = self.clip.encode_image(self.preprocess(target))
        x_enc = self.clip.encode_image(self.preprocess(x))
        return self.mse(target_enc.detach(), x_enc)
